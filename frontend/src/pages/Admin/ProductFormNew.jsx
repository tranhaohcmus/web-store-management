...
function ProductFormNew() {
  ...
  // Load data (brands, categories, product types)
  useEffect(() => {
        ...
        if (res.success && res.data) {
          ...
          if (product.variants && product.variants.length > 0) {
            const loadedVariants = product.variants.map((v) => {
              // Extract attributes from variantAttributeValues
              const attributes = {};
              if (v.variantAttributeValues) {
                v.variantAttributeValues.forEach((vav) => {
                  attributes[vav.attribute_id] = vav.attribute_value_id;
                });
              }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditMode && !imageFile) newErrors.image = "Bắt buộc";
    ...
    try {
      ...
      alert(error.response?.data?.message || "Lỗi xử lý sản phẩm");
    } finally {
      setLoading(false);
    }
  };
